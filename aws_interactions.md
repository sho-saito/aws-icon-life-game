...
進化は、同種のアイコンが隣接した状態を維持すると、合体して別のアイコンに変化する関係です。進化後のアイコン（AutoScaling）はランダム配置では出現せず、進化によってのみ登場します。

| 進化前 | 進化後 | 条件 | 説明 |
|-------|-------|------|------|
| EC2 ×3 | AutoScaling ×1 | 3つ以上のEC2が隣接（中心間80px以内）した状態が180フレーム（約3秒）継続 | 複数のEC2インスタンスをまとめて管理するAuto Scalingへの移行を表現しています。進化中のEC2には進行度を示す青い円弧が表示され、条件を満たすと3つのEC2が重心位置で1つのAutoScalingに合体します。 |
